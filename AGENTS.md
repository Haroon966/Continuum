...
If the user pastes the pinky-cursor GitHub link and wants Pinky in their project: temp-clone the kit → run `install.sh` into the project → delete the clone. Keep **only** payload files — no nested kit checkout, no kit README/assets/CI/scripts left behind. Full steps: kit `INSTALL.md` on GitHub.
...
5. Never bulk-attach all of `.cursor/skills/` or the kit README "for safety".
